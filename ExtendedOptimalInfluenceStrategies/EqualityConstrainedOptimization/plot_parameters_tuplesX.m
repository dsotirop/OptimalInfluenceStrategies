...
if(varying_parameters_num==1)
    ...
    title(TitleName);
    % 6th Figure.
    Figure6Name = strcat(['T0 Optimal with respect to ' ParamName]);
    figure('Name',Figure6Name);
    hold on
    plot(Param,T0,'*c','LineWidth',1.4);
    xlabel(ParamName);
    ylabel('T0opt');
    grid on
    title(TitleName);    
...
if(varying_parameters_num==2)
    ...
    title(TitleName);
    % 10th Figure.
    Figure10Name = strcat(['T0 Optimal with respect to ' Param1Name ' and ' Param2Name]);
    figure('Name',Figure10Name);
    plot3(Param1,Param2,T0,'*c','LineWidth',1.4);
    hold on
    plot3(Param1,Param2,Lambda2,'*g','LineWidth',1.4);
    xlabel(Param1Name);
    ylabel(Param2Name);
