...
% the First Order and Second Order requirements as well as the positiveness 
% of all the quantities of the model. Notice that the optimal Lagrange 
% multiplier solutions have been discarded.
% -------------------------------------------------------------------------
% IMPORTANT NOTE:
% -------------------------------------------------------------------------
...
% the necessary first and second order conditions. However, an erro will be 
% raised in case the extracted representative solution violates the
...
for dim_index = 1:Dimensionality
    ...
    if(max_dist > 10^(-MinimumDigitsAccuracy+1))
        ...
        % Check whether the maximum distance is less than the smallest real
        % number that can be represented by Matlab. In that case, the
        % digits accuracy is assumed to be infinite.
        if(max_dist < eps)
