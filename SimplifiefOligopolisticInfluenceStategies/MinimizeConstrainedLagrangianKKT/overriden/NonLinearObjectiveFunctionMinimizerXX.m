...
% equality constraints at the solution points.
Cineq = zeros(N,2);
Ceq = zeros(N,4);
...
options = optimoptions(@fmincon,'Algorithm','sqp','Display',DisplayFlag,'ConstraintTolerance',tolerance_value,'MaxIterations',MaxIterations,'MaxFunctionEvaluations',MaxFunctionEvaluations,'StepTolerance',tolerance_value,'OptimalityTolerance',tolerance_value);
...
%                   TA + TB + 0 * LambdaA_1 + 0 * LambdaA_2 + 0 * LambdaB_1 + 0 * LambdaB_2 <= 1 [I]
%
% Given that T = [TA;TB;LambdaA_1;LambdaA_2;LambdaB_1;LambdaB_2], matrices A and B will be of the following form:
%
% A = [1 1 0 0 0 0] and B = [1].

A = [1 1 0 0 0 0]; % A should be a [1 x 6] matrix.
