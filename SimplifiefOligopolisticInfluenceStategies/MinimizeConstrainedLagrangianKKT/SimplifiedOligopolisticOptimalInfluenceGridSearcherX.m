...
% Set the minimum and maximum values for M.
m_min = 0.00;
m_max = 1.00;
% Set the minimum and maximum values for K.
k_min = 0.40;
k_max = 0.60;
% Set the minimum value of the LA and LB parameters.
l_min = 0.40;
% Compute an upper threshold for the G value.
alpha0 = (k_max*m_max - 2) / (m_min^2 - 4);
beta0 = (2*k_max - m_min) / (m_min^2 - 4);
Fmax = max(alpha0^2,(3*beta0^2+2*alpha0*beta0));
G0 = Fmax  / (l_min^2);
...
parfor grid_index = 1:1:RangeLength
...
% remain constant.
ConstIndices = setdiff(1:ParamsNum,ParamIndex);
ConstValues = Params(1,ConstIndices);

% Set the name of the experimentation folder.
ExperimentFolder = 'experiments';

% Set the experiment index used to save the obtained results. Mind that
% each experiment index identifies a particular experiment within the
% series of different experiementation scenarios where a particular
% parameter is allowed to vary.
ExperimentIndex = 6;

% Set the structure of stored variables for the current experiment.
Experiment.Description = 'Varying_M';
Experiment.Parameters = PARAMETERS;
Experiment.ParamIndex = ParamIndex;
Experiment.Params = Params;
Experiment.ParamsNames = ParamsNames;
Experiment.ParamsNum = ParamsNum;
Experiment.ConstValues = ConstValues;
Experiment.ConstIndices = ConstIndices;
Experiment.Topts = Topts;
Experiment.Sopts = Sopts;
Experiment.Xopts = Xopts;
Experiment.Popts = Popts;
Experiment.Qopts = Qopts;
Experiment.Fopts = Fopts;

% Set the filename.
FileName = sprintf('%s_%d',Experiment.Description,ExperimentIndex);
% Set the full filename.
FullFileName = fullfile(ExperimentFolder,FileName);
% Save the current experiment.
save(FullFileName,'Experiment');

% Perform plotting operations.
PlotParametersTuples(Experiment.Topts,Experiment.Sopts,Experiment.Xopts,...
                     Experiment.Popts,Experiment.Qopts,Experiment.Fopts,...
                     Experiment.Params,Experiment.ParamIndex,...
                     Experiment.ConstIndices,Experiment.ConstValues,MinimumDigitsAccuracy);