...
% lies upon the utilization of the "Quality and Competition" model which
% was originally proposed by Banker et al.

% For this particular formulation we utilize the original functional form
% for the demand function which is parameterized by competition-related 
% factors Gx,Gy and Go as well as the quality and price responsiveness
% factors K and M.
...
%
% Qx = Kx*A + (Xx - K*Xy) - (Px - M*Py) [1]
% Qy = Ky*A + (Xy - K*Xx) - (Py - M*Px) [2]
...
%    Ry = (Py - C - V*Xy) * Qy [4]
%
% Mind that (C) denotes the the marginal cost and (V) is utilized in order
% to quantify quality-related costs.

% Cost functions Cx and Cy which will be defined according to Banker et al.
% as:
%                    2                        2
%    Cx = Go + G * Xx  [5] and Cy = Go+ G * Xy  [6]
...
syms Xx Xy M K V Px Py Kx Ky A G C
...
% Qx = Kx*A + (Xx - K*Xy) - (Px - M*Py);
% Qy = Ky*A + (Xy - K*Xx) - (Py - M*Px);
Qx = (Xx - K*Xy) - (Px - M*Py);
Qy = (Xy - K*Xx) - (Py - M*Px);
...
% % Comment out to cancel model simplification.
% Rx = subs(Rx,[C V A],[0 0 0]);
% Ry = subs(Ry,[C V A],[0 0 0]);
